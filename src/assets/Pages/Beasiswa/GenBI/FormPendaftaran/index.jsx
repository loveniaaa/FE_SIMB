...
function FormPendaftaranGenBI() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validasi NIM sesuai dengan Fakultas
    const nimFacultyCode = nim.slice(0, 4); // ambil 4 digit fakultas dari NIM
    console.log("nimFacultyCode : " + nimFacultyCode);
    console.log("facultyCode : " + facultyCode);
    if (nimFacultyCode !== facultyCode) {
      // setError(`NIM tidak sesuai dengan fakultas yang dipilih. Kode dari NIM adalah ${nimFacultyCode}, tapi Anda memilih ${facultyCode}`);
      setError(`NIM tidak sesuai dengan fakultas yang dipilih.`);
      return;
    }
    ...
    try {
      const response = await apiClient.post("/scholarship/create", requestBody, {
        headers: {
